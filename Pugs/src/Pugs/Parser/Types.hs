...
enterBracketLevel bracket rule = do
    prev <- s_bracketLevel `fmap` getState
    updateState $ \state -> state{ s_bracketLevel = bracket }
    rv <- rule
    updateState $ \state -> state{ s_bracketLevel = prev }
    return rv
...
getRuleEnv = s_env `fmap` getState
...
modifyRuleEnv f = updateState $ \state -> state{ s_env = f (s_env state) }
...
addBlockPad pad = do
    ...
    setState state{ s_knownVars = s_knownVars state `Map.union` myKnownVars }
...
clearDynParsers = updateState $ \state -> state{ s_dynParsers = MkDynParsersEmpty }
